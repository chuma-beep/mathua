...
Given a quadratic equation in standard form \\(ax^2 + bx + c = 0\\), the quadratic formula provides an explicit expression for its roots in terms of the coefficients \\(a\\), \\(b\\), and \\(c\\):

\\[
x_{1,2} = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}
\\]

The formula is derived by applying the method of completing the square to the general standard form, and constitutes one of the central results of algebra. It is universally applicable to any quadratic equation with real or complex coefficients, provided that \\(a \neq 0\\).
...
A detailed treatment of this case is presented in the entry on quadratic equations with complex solutions.


The discriminant \\(\Delta = b^2 - 4ac\\) also determines how the graph of the quadratic function \\(f(x) = ax^2 + bx + c\\) behaves with respect to the x-axis. Geometrically, a quadratic equation represents a parabola. Consider for example the following equation \\( y = x^2 + 4x + 4 \\).
