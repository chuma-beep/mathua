...
Add to both sides: if $2\lt4$, then $2\addright{1}\confirm{\lt}4\addright{1}$.

Subtract from both sides: if $2\lt4$, then $2\subtractright{1}\confirm{\lt}4\subtractright{1}$.

Multiply on both sides by a *positive* number: if $2\lt4$, then $\multiplyleft{3}2\confirm{\lt}\multiplyleft{3}4$.
...
There are at least three ways to represent an inequality's solution set:  graphically, with set-builder notation, and with interval notation.  (Interval notation and set-builder notation are discussed in  the section .) Graphically, the solution set is part of a number line:
...
$$
\begin{aligned}-2x&\geq12 \\ \divideunder{-2x}{-2}&\secondhighlight{\leq}\divideunder{12}{-2}&&\text{Note the change in direction.} \\ x&\leq-6\end{aligned}
$$
...
$$
\begin{aligned}&& -2x&\ge12&& \\ -2(\substitute{-7})&\wonder{\geq}12& -2(\substitute{-6})&\wonder{\ge}12& -2(\substitute{-5})&\wonder{\ge}12 \\ 14&\confirm{\geq}12& 12&\confirm{\ge}12& 10&\reject{\ge}12\end{aligned}
$$
