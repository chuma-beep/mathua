...
A fundamental property of real numbers is that rational approximations are compatible with arithmetic operations, and, in particular, with multiplication. This means that, if  and  are positive real numbers such that \(a=\sup_{x\in A} x\) and \(b=\sup_{y\in B} y,\) then \(a\cdot b=\sup_{x\in A, y\in B}x\cdot y.\) In particular, the product of two positive real numbers is the least upper bound of the term-by-term products of the sequences of their decimal representations.
