...
where is the imaginary unit (*i*\(^{2}\) ). The formula is named after Abraham de Moivre, although he never stated it in his works. The expression cos *x* + *i* sin *x* is sometimes abbreviated to cis *x*.
...
As written, the formula is not valid for non-integer powers. However, there are generalizations of this formula valid for other exponents. These can be used to give explicit expressions for the th roots of unity, that is, complex numbers such that *z\(^{n}\)*. Using the standard extensions of the sine and cosine functions to complex numbers, the formula is valid even when is an arbitrary complex number.
...
If is a rational number (but not necessarily an integer), then cosh *nx* + sinh *nx* will be one of the values of (cosh *x* + sinh *x*)\(^{*n*}\).
