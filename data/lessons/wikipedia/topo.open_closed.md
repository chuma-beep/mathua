...
### Euclidean space
A subset \(U\) of the Euclidean *n*-space **R**\(^{*n*}\) is *open* if, for every point in \(U\), there exists a positive real number (depending on ) such that any point in **R**\(^{*n*}\) whose Euclidean distance from is smaller than belongs to \(U\). Equivalently, a subset \(U\) of **R**\(^{*n*}\) is open if every point in \(U\) is the center of an open ball contained in \(U.\)
