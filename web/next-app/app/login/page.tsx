      ...
      <div className="max-w-container mx-auto px-6 max-sm:px-4">
        <section className="pt-8 max-w-md mx-auto mt-12">
          ...
          <div className="flex gap-2 mt-6 mb-4">
            <button onClick={() => { setTab('login'); setError('') }} className={`flex-1 rounded-none h-12 text-sm font-medium ${tab === 'login' ? 'bg-mathua-blue text-white' : 'bg-mathua-surface-elevated border border-mathua-border text-mathua-secondary'}`}>Login</button>
            <button onClick={() => { setTab('signup'); setError('') }} className={`flex-1 rounded-none h-12 text-sm font-medium ${tab === 'signup' ? 'bg-mathua-blue text-white' : 'bg-mathua-surface-elevated border border-mathua-border text-mathua-secondary'}`}>Sign Up</button>
          </div>
          <div className="bg-mathua-surface border border-mathua-border rounded-none p-6 space-y-4">
            {tab === 'signup' && (
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSubmit()} placeholder="Your name" autoFocus className="w-full mt-1 bg-mathua-code border border-mathua-border rounded-none h-12 px-4 font-mono text-base text-mathua-primary placeholder:text-mathua-muted focus:outline-none focus:border-mathua-blue" />
              </div>
            )}
            <div>
              <label className="font-mono text-[10px] uppercase text-mathua-muted">Username</label>
              <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSubmit()} placeholder="username" className="w-full mt-1 bg-mathua-code border border-mathua-border rounded-none h-12 px-4 font-mono text-base text-mathua-primary placeholder:text-mathua-muted focus:outline-none focus:border-mathua-blue" />
            </div>
            <div>
              <label className="font-mono text-[10px] uppercase text-mathua-muted">Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSubmit()} placeholder="password" className="w-full mt-1 bg-mathua-code border border-mathua-border rounded-none h-12 px-4 font-mono text-base text-mathua-primary placeholder:text-mathua-muted focus:outline-none focus:border-mathua-blue" />
            </div>
            {error && <p className="text-mathua-red text-xs">{error}</p>}
            <button onClick={handleSubmit} disabled={loading} className="w-full bg-mathua-blue text-white hover:bg-mathua-blue-hover rounded-none h-12 font-medium text-sm disabled:opacity-50">
